...
open class LiveEventListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, SBUEmptyViewDelegate {
    ...
    open var theme: SBUChannelListTheme = .init()
    ...
    func showLiveEvent(_ liveEvent: LiveEvent) {
        ...
        SendbirdLive.getLiveEvent(id: liveEvent.liveEventId) { result in
            DispatchQueue.main.async {
                ...
                if liveEvent.myRole == .host && liveEvent.host == nil {
                    liveEvent.enterAsHost(options: .init(turnVideoOn: true, turnAudioOn: true)) { _ in
                        ...
                        liveEvent.startStreaming(mediaOptions: nil) { _ in
                            DispatchQueue.main.async {
                                guard let channel = liveEvent.openChannel else { return }
                                
                                
                                let vc = HostViewController(channel: channel, liveEvent: liveEvent)
    ...
    func enterLiveEvent(_ liveEvent: LiveEvent) {
        liveEvent.enter { error in
            DispatchQueue.main.async {
                ...
                let vc = ParticipantViewController(channel: channel, liveEvent: liveEvent)
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }
    }
    ...
    @objc func clickGoLive(_ sender: Any) {
        SBULoading.start()
        var config = LiveEvent.CreateParams(hostType: .singleHostAudioOnly)
        config.title = "\(SendbirdChat.getCurrentUser()!.nickname)'s audio live"
        config.coverURL = SendbirdChat.getCurrentUser()!.profileURL
        config.reactionKeys = ["LIKE"]
        
        SendbirdLive.createLiveEvent(
            config: config
        ) { result in
            switch result {

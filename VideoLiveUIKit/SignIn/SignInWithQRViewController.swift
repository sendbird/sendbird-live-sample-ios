...
extension SignInWithQRViewController {
    ...
    func signIn(with credential: Credential) {
        ...
        UserDefaults.standard.set(credential.userId, forKey: "userId")
        UserDefaults.standard.set(credential.accessToken, forKey: "accessToken")
        UserDefaults.standard.set(credential.appId, forKey: "applicationId")

        
        SBUGlobals.currentUser = .init(userId: credential.userId)
        SBUGlobals.accessToken = credential.accessToken
        SBUGlobals.applicationId = credential.appId
...
//        let voipPushToken = UserDefaults.standard.voipPushToken
        ...
        // Update app ID
        SendbirdLiveUI.initialize(applicationId: credential.appId, startHandler: nil, migrationHandler: nil) { error in
